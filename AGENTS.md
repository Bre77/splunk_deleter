...
- `default/app.conf`'s `[launcher] version` and `[id] version` must move together; the app's top-level package dir name is `deleter` (see `.build.sh`'s final `tar` target and `package.json`).
